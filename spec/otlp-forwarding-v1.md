...
`destination_url` is customer-supplied, so each run first calls `assertPublicHttpDestination` (`apps/worker/src/lib/ssrf-guard.ts`), the same guard webhooks use. It requires `http` or `https`, resolves the hostname, and rejects the destination if any resolved address is refused; it checks the resolved addresses, not only the hostname string. The address rules are described in `spec/webhooks-v1.md`, "SSRF guard": everything that is not globally reachable, including the shared address space `100.64/10` and IPv6 forms carrying a private IPv4 address.

A rejected destination fails the run before any request is sent; the run is recorded on the rule, the scheduler reports the error, and the rule is retried on its own interval. Requests do not follow redirects: a 3xx response fails the trace like a 5xx, because its target was never checked. Requests go through `publicFetch`, which checks the address again when each connection is made: the hostname is resolved once, every address in the answer is checked, and the socket connects to those addresses, so a hostname that resolves to a public address for the run's check and a private one for the request (DNS rebinding) is refused with no request sent. An IP address in the URL is checked the same way, TLS still verifies the certificate against the hostname, and pooled connections were checked when they were opened. Deliveries connect directly: `HTTP_PROXY`, `HTTPS_PROXY` and `NODE_USE_ENV_PROXY` are ignored, since a proxy would make the connection-time check meaningless. Tests opt out with `allowPrivateDestinations: true` to reach a local server; they then send through `anyAddressFetch`, the same transport refusing no address, so the tests exercise the production client. The scheduler never sets it.

## Verified

`apps/worker/test/otlp-forwarder.test.ts` publishes traces to the feed and runs `forwardOtlpTraces` against real Postgres and ClickHouse and a local HTTP server. It covers one request per trace with the auth header and nested spans, a 500 stopping the run and the next run resuming at the same trace, a 413 skipped and recorded while the next trace is forwarded, a 401, 403 or 404 stopping the run without skipping anything, a request timeout stopping the run, filter matching, the SSRF guard rejecting a loopback destination with no request sent, and a destination that rebinds between the run's check and the connection failing the trace with no request sent. `apps/worker/test/otlp-mapper.test.ts` covers id length and stability, parent/child links, the `gen_ai.*` attributes, error status, the synthetic root span, and nanosecond timestamps. `apps/worker/test/ssrf-guard.test.ts` covers the address rules (every refused range at its edges, public neighbours, IPv6 forms carrying an IPv4 address, numeric URL forms of IPv4), the guarded lookup for single-address and list lookups, and `publicFetch` against a local server: a hostname answered with a private address at connect time (over http and https), an answer mixing public and private addresses, IP addresses in the URL, and `localhost` are refused with no request received; with loopback allowed, the request reaches the server with its hostname, and a redirect is returned, not followed.
...
- Still open: forwarding follows the scheduler's poll cadence (default every 5 minutes, after the quiet period), not the moment a trace settles; forwarding on ingest would change the ingest pipeline and is not built. Only the `Authorization` header can be configured.
- The SSRF guard used to check the destination once per run while each request resolved the hostname again, so DNS rebinding between the two was not caught. Requests now go through `publicFetch` (`spec/webhooks-v1.md`, "SSRF guard").
