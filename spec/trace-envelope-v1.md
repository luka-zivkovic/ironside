...
- Upsert semantics: same id twice = update (ClickHouse ReplacingMergeTree handles dedup by event timestamp). A record is a trace by id, or an observation or score by trace id and id.
  - The day of a trace's or score's timestamp, and of an observation's start time, is part of its ClickHouse sort key, and ReplacingMergeTree replaces a row only under the same key. So before writing a batch, the ingest worker (`apps/worker/src/processors/moved-rows.ts`):
    ...
    Deletions are written after the rows, so a failure in between leaves a duplicate that the retry removes, never a missing record. Days are compared only between 1970-01-01 and 2149-06-06, the range of the Date type the sort key uses, and only when the ClickHouse server's timezone is UTC, as in the shipped Compose files. Outside that range `toDate` does not return the calendar day, and in another timezone its day can differ from the UTC day (a daylight-saving jump at midnight moves a time to the previous day). So such rows are never deleted as moved and never share a key with a row the batch writes; a duplicate can remain there, a record cannot be lost. Imports delete every earlier row of a trace they rewrite.
