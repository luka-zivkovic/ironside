...
- Images are exact: `ghcr.io/luka-zivkovic/ironside-{web,api,worker}:${IRONSIDE_VERSION}`, `postgres:16.15-alpine`, `clickhouse/clickhouse-server:25.3.14.14-alpine`, `redis:7.4.7-alpine` and MinIO as Chainguard's build of `RELEASE.2026-09-22T19-25-18Z` pinned by digest (`cgr.dev/chainguard/minio@sha256:…`). MinIO no longer publishes public images. The MinIO service runs as root (`user: "0"`), as the official image did, because the Chainguard image's default user cannot write a data volume the official image created.
...
- 0.3.1 moved MinIO from `quay.io/minio/minio`, which stopped serving anonymous pulls, to Chainguard's build. Before the release, a v0.3.0 bundle was booted with the published 0.3.0 images, an object was written, and the bundle was switched to the new MinIO image: MinIO started on the same volume, the object read back and the API reported object storage healthy. A fresh install from the new bundle also booted healthy. Chainguard's free images are published under `latest`, so an old digest may eventually stop being pullable; the next image move needs the same drill.
