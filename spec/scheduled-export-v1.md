...
- **Versions:** a trace that receives new activity after being exported moves to the end of the feed and is exported again. Every row carries `trace_version`, the snapshot's latest trace/observation activity; consumers keep the newest version per id.
...
- **Delivery:** the position advances only after every file of the run is uploaded, so a failed run sends the same traces again. Delivery is at-least-once.
