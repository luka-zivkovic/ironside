...
- A change to scores alone replaces this provider's imported scores and notifies the score feed without reopening the trace. The score feed is published before the pending snapshot is cleared, so a run that stops in between is repeated by recovery and loses no notification. Imported scores carry `import_source = 'langfuse'`, so native, manual and evaluator scores on the same trace are never replaced.
