...
- A failure after the response arrived (a body that is cut off or does not parse, an abort while it downloads) is recorded once as an error by hooking the result's `parseResponse`, which the SDK calls when the result is read, also for the promises `.withResponse()` and `parse()` derive from it.
