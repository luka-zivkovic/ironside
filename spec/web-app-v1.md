...
- Trace detail lives at the stable, deep-linkable route `/projects/:projectId/traces/:traceId`. When the API's optional `IRONSIDE_COEVAL_URL` is set, the owner-session `GET /api/v1/viewer-config` returns it at runtime and the header shows an "Open in Coeval" link; otherwise the view is unchanged. Both link directions are specified in [`evaluator-integration-v1.md`](./evaluator-integration-v1.md#viewer-deep-links).
