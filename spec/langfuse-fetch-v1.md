...
- A numeric `value` becomes a `numeric` score, or `boolean` when `dataType` is `BOOLEAN` (any case). A string `value` becomes a `categorical` score with `stringValue`, even when `dataType` says `BOOLEAN`. `source` is always `api`. Metadata values that are not strings are JSON-stringified. The score's `timestamp` is the receive time. Without an `id`, an `Idempotency-Key` header (1 to 255 characters after trimming; 400 otherwise) names the score together with its content: the id is `idem_` and the first 32 hex characters of the SHA-256 of the key and the parsed score body as canonical JSON (keys sorted). A client resending the same request replaces the score instead of adding one, and a key reused for a different score, such as one key per workflow run posting several metrics, makes a distinct score rather than overwriting the first. Without either, every request is a new score, since two identical scores can both be real. With a body `id` the header is ignored and not validated.
