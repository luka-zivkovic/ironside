...
- A numeric `value` becomes a `numeric` score, or `boolean` when `dataType` is `BOOLEAN` (any case). A string `value` becomes a `categorical` score with `stringValue`, even when `dataType` says `BOOLEAN`. `source` is always `api`. Metadata values that are not strings are JSON-stringified. The score's `timestamp` is the receive time. Without an `id`, an `Idempotency-Key` header (1 to 255 characters; 400 otherwise) names the score: the id is `idem_` and the first 32 hex characters of its SHA-256, so a client resending the same request replaces the score instead of adding one. Without either, every request is a new score, since two identical scores can both be real; a body `id` takes precedence over the header.
