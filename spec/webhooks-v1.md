...
`destinationUrl` is customer-supplied, so each run first resolves it (`apps/worker/src/lib/ssrf-guard.ts`, `assertPublicHttpDestination`), checking the resolved addresses rather than the hostname string, and rejects any address that is not globally reachable (`isPrivateOrReservedAddress`, following the IANA special-purpose address registries): loopback, private (`10/8`, `172.16/12`, `192.168/16`, `fc00::/7`), shared address space (`100.64/10`, used for carrier-grade NAT and by some clouds' metadata services), link-local (`169.254/16`, including the cloud metadata address `169.254.169.254`, and all of `fe80::/10`), `0/8`, unspecified, benchmarking, documentation, IETF protocol assignments, multicast, and reserved (`240/4`, including broadcast). An IPv6 address carrying an IPv4 address (IPv4-mapped, IPv4-translated, IPv4-compatible, NAT64 `64:ff9b::/96` and 6to4 `2002::/16`) is judged by that IPv4 address, in any textual form (Node's `URL` parser turns `::ffff:172.20.1.1` into `::ffff:ac14:101`), so NAT64 and 6to4 still reach public hosts. Anything that is not an IP address is refused. A rejected destination fails the run and is recorded on the rule. Requests do not follow redirects: a 3xx response is a failed delivery, because its target was never checked. Requests go through `publicFetch`, which checks the address again when each connection is made: the hostname is resolved once, every address in the answer is checked, and the socket connects to those addresses, so a hostname that resolves to a public address for the run's check and a private one for the request (DNS rebinding) is refused with no request sent. An IP address in the URL is checked the same way, TLS still verifies the certificate against the hostname, and pooled connections were checked when they were opened. Deliveries connect directly: `HTTP_PROXY`, `HTTPS_PROXY` and `NODE_USE_ENV_PROXY` are ignored, since a proxy would make the connection-time check meaningless. Tests opt out with `allowPrivateDestinations: true` to reach a local server; they then send through `anyAddressFetch`, the same transport refusing no address, so the tests exercise the production client. Production code never sets it.
...
`apps/worker/test/webhook-runner.test.ts` publishes traces to the feed and runs `runWebhooks` against real Postgres, ClickHouse and a local HTTP server. It covers the signed payload and the recorded run, no resend from a later run and no position change from a stale one, a new webhook for each republication (including a late batch), a failed delivery stopping the run and being retried first in feed order, stopping at an in-flight delivery, the handoff from a previous-release worker (holding the old key during a send, releasing it after a failure, covering it after a success, and entries inside and after the 24-hour window), a filter matching nothing, a bookkeeping failure after a confirmed delivery, the SSRF guard, and a destination that rebinds (the per-run check resolving it to a public address, the connection to loopback) failing the delivery with no request sent, which fails if the runner sends with anything but `publicFetch`. `apps/worker/test/ssrf-guard.test.ts` covers the address rules and `publicFetch` (`spec/otlp-forwarding-v1.md`, "Verified"). `packages/db/test/webhooks.test.ts` covers the claim itself (including 10 parallel claims producing one winner), a covered key refusing the previous release's claim, and the guarded position update. `packages/db/test/migrate-upgrade.test.ts` checks that existing rules are handed off at the migration.
...
- The SSRF guard used to check the destination once per run while each request resolved the hostname again, so DNS rebinding between the two was not caught. Deliveries now go through `publicFetch`, which checks the address each connection is made to.
