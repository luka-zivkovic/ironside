...
- `batch[].id` is the envelope id of the ingestion event. `batch[].body.id` is the trace, observation or score id, and is what upserts; surrounding whitespace is trimmed, so events whose ids differ only by it belong to one record. Event ids are not required to be unique, so they never identify a record on their own.
...
- A record sent without an `id` gets one derived from its event: `lf_` and the first 32 hex characters of a SHA-256 of the event's type, id, timestamp and body (keys sorted). A worker retry replays the stored batch and a client resend repeats the event, so both write the same record again rather than a copy, while a client that reuses an event id for different records still gets distinct ones. An event without a body id is never grouped with another event. The derived id deduplicates by content: a resend must repeat the event's id, timestamp and body exactly to replace the first write, and two identical events (same id, timestamp and body) are one record, so a client that reuses event ids and sends no timestamp cannot record the same score twice this way; it should send body ids. The LangFuse JS SDK always sends body ids; this covers other clients.
- A group of events that fails to map for an unexpected reason (a body nested too deeply to serialize, for example) is reported as failed for its own events only; the rest of the request is still mapped.
...
- The worker merges each incoming row into the stored one field by field (`apps/worker/src/processors/langfuse-merge.ts`), in any processing order:
  ...
  - **Moved rows are deleted.** The day of a trace's timestamp and of an observation's start time is part of its ClickHouse sort key. When a merge moves one to another day (a late create replacing a placeholder across midnight UTC), the record's stored rows under another day, at or below the merged row's version, are written as deletions with that version after the merged row, so one row remains. Which days are compared is limited as described in `spec/trace-envelope-v1.md` ("Upsert semantics").
