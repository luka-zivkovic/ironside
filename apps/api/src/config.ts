...
export interface Config {
  ...
  /** Optional Coeval web base URL; when set, the trace viewer links each trace to Coeval. */
...
// An absolute http(s) base, optionally with a path prefix for a Coeval served
// under a subpath. Query, fragment, and userinfo are rejected because the
// viewer appends its own path and query to this value.
export function optionalCoevalUrl(value: string | undefined): string | null {
  ...
  try {
    ...
    throw new Error(`IRONSIDE_COEVAL_URL must be an absolute http(s) URL: ${trimmed}`);
  }
  if (
    ...
    throw new Error(
      `IRONSIDE_COEVAL_URL must be an absolute http(s) URL without credentials, query, or fragment: ${trimmed}`
    );
  }
...
export function loadConfig(env: NodeJS.ProcessEnv = process.env): Config {
  return {
    ...
    coevalUrl: optionalCoevalUrl(env.IRONSIDE_COEVAL_URL)
  };
}
