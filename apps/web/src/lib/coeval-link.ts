// Shared cross-product deep-link contract with Coeval. Coeval resolves the
...
export interface CoevalTraceLinkInput {
  projectId: string;
  traceId: string;
  traceVersion?: string | null;
}

export function coevalTraceLink(coevalUrl: string, input: CoevalTraceLinkInput): string {
  ...
  return `${coevalUrl.replace(/\/+$/, "")}/links/trace?${search.toString()}`;
}
