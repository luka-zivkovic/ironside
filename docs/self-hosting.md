...
| `IRONSIDE_COEVAL_URL` | unset | Optional Coeval web base URL (API only), such as `https://coeval.example.com`. When set, each trace view shows an **Open in Coeval** link; unset leaves the viewer unchanged. Must be an absolute `http(s)` URL without credentials, query, or fragment. The web app reads it at runtime, so no image rebuild is needed. Link shapes: [`spec/evaluator-integration-v1.md`](../spec/evaluator-integration-v1.md#viewer-deep-links) |
